...
-- audit-grade record of exactly what was generated for that cycle —
-- probabilities, fixed coefficients, the winning bulb, and the full
-- elimination order (pop order for every other bulb) — everything needed
-- to independently re-run an RTP simulation later.
-- ----------------------------------------------------------------------------
create table if not exists cycles (
  ...
  shape text not null check (shape in ('dominant', 'wide_open', 'duel')),
  -- bulb_id -> probability, e.g. {"bulb_1": 0.42, "bulb_2": 0.13, ...}
  probabilities jsonb not null,
  -- bulb_id -> fixed (HOUSE_RTP / probability, clamped) coefficient
  fixed_coefficients jsonb not null,
  ...
  status text not null default 'betting' check (status in ('betting', 'active', 'complete')),
